...
namespace noise
{
  ...
  namespace utils
  {
      ...
      const std::uint8_t channel1, float alpha)
    {
      ...
      return static_cast<std::uint8_t>(((c1 * alpha) + (c0 * (1.0f - alpha))) * 255.0f);
    }
    ...
    // hope.)
    inline std::uint8_t* UnpackFloat (std::uint8_t* bytes, float value)
    {
      std::uint8_t* pBytes = reinterpret_cast<std::uint8_t*>(&value);
      bytes[0] = *pBytes++;
      bytes[1] = *pBytes++;
      bytes[2] = *pBytes++;
      bytes[3] = *pBytes++;
    ...
    inline std::uint8_t* UnpackLittle16 (std::uint8_t* bytes,
      std::uint16_t integer)
    {
      bytes[0] = static_cast<std::uint8_t>((integer & 0x00ff)      );
      bytes[1] = static_cast<std::uint8_t>((integer & 0xff00) >> 8 );
      return bytes;
    }

    // Unpacks a 32-bit integer value into four bytes in little endian format.
    inline std::uint8_t* UnpackLittle32 (std::uint8_t* bytes,
      std::uint32_t integer)
    {
      bytes[0] = static_cast<std::uint8_t>((integer & 0x000000ff)      );
      bytes[1] = static_cast<std::uint8_t>((integer & 0x0000ff00) >> 8 );
      bytes[2] = static_cast<std::uint8_t>((integer & 0x00ff0000) >> 16);
      bytes[3] = static_cast<std::uint8_t>((integer & 0xff000000) >> 24);
      return bytes;
    }
...
const Color& GradientColor::GetColor (double gradientPos) const
{
  ...
  int index0 = std::clamp (indexPos - 1, 0, m_gradientPointCount - 1);
  int index1 = std::clamp (indexPos    , 0, m_gradientPointCount - 1);
...
void NoiseMap::SetSize (int width, int height)
{
  if (width < 0 || height < 0
    ...
    m_stride = static_cast<int>(CalcStride (width));
    m_width  = width;
    m_height = height;
  }
}
...
void WriterBMP::WriteDestFile ()
{
  ...
  std::uint8_t* pLineBuffer = nullptr;
  ...
  try {
    pLineBuffer = new std::uint8_t[bufferSize];
  }
  catch (...) {
    throw noise::ExceptionOutOfMemory ();
  }
  ...
  os.write ("\0\0\0\0", 4);
  os.write ((char*)UnpackLittle32 (d, (std::uint32_t)BMP_HEADER_SIZE), 4);
  os.write ((char*)UnpackLittle32 (d, 40), 4);   // Palette offset
  os.write ((char*)UnpackLittle32 (d, (std::uint32_t)width ), 4);
  os.write ((char*)UnpackLittle32 (d, (std::uint32_t)height), 4);
  ...
  os.write ((char*)UnpackLittle32 (d, (std::uint32_t)destSize), 4);
...
{
  return (width * sizeof (std::int16_t));
}
...
void WriterTER::WriteDestFile ()
{
  ...
  // Build the header.
  std::uint8_t d[4];
  std::int16_t heightScale = static_cast<std::int16_t>(std::floor(32768.0 / m_metersPerPoint));
  os.write ("TERRAGENTERRAIN ", 16);
  os.write ("SIZE", 4);
  os.write ((char*)UnpackLittle16 (d, std::min (width, height) - 1), 2);
  os.write ("\0\0", 2);
  os.write ("XPTS", 4);
  ...
  for (int y = 0; y < height; ++y) {
    ...
    std::uint8_t* pDest   = pLineBuffer;
    for (int x = 0; x < width; ++x) {
      std::int16_t scaledHeight = static_cast<std::uint16_t>(std::floor (*pSource * 2.0));
  ...
  const Color& backgroundColor, double lightValue) const
{
  ...
  if (m_isLightEnabled) {
    ...
    double lightRed { lightValue * m_lightColor.red   / 255.0 };
    double lightGreen{ lightValue * m_lightColor.green / 255.0 };
    double lightBlue{ lightValue * m_lightColor.blue  / 255.0 };
  ...
  // Rescale the color channels to the std::uint8_t (0..255) range and return
  // the new color.
  Color newColor (
    static_cast<std::uint8_t>(static_cast<std::uint8_t>((red   * 255.0)) & 0xff),
    static_cast<std::uint8_t>(static_cast<std::uint8_t>((green * 255.0)) & 0xff),
    static_cast<std::uint8_t>(static_cast<std::uint8_t>((blue  * 255.0)) & 0xff),
    std::max (sourceColor.alpha, backgroundColor.alpha));
  return newColor;
}
  ...
  for (int y = 0; y < height; ++y) {
    ...
    for (int x = 0; x < width; ++x) {
      ...
      if (m_isLightEnabled) {
        ...
        if (m_isWrapEnabled) {
          if (x == 0) {
            xLeftOffset  = static_cast<int>(width) - 1;
            xRightOffset = 1;
          } else if (x == static_cast<int>(width) - 1) {
            xLeftOffset  = -1;
            xRightOffset = -(static_cast<int>(width) - 1);
          } else {
            xLeftOffset  = -1;
            xRightOffset = 1;
          }
          if (y == 0) {
            yDownOffset = static_cast<int>(height) - 1;
            yUpOffset   = 1;
          } else if (y == static_cast<int>(height) - 1) {
            yDownOffset = -1;
            yUpOffset   = -(static_cast<int>(height) - 1);
          } else {
            yDownOffset = -1;
            yUpOffset   = 1;
          }
  ...
  double bumpHeight) const
{
  ...
  std::uint8_t xc{ static_cast<std::uint8_t>(static_cast<std::uint8_t>(((std::floor)((vxc + 1.0) * 127.5))) & 0xff) };
  std::uint8_t yc{ static_cast<std::uint8_t>(static_cast<std::uint8_t>(((std::floor)((vyc + 1.0) * 127.5))) & 0xff) };
  std::uint8_t zc{ static_cast<std::uint8_t>(static_cast<std::uint8_t>(((std::floor)((vzc + 1.0) * 127.5))) & 0xff) };
